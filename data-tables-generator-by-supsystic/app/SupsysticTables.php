...
class SupsysticTables
{
    ...
    public function __construct()
    {
        ...
        $environment = new RscDtgs_Environment("st", "1.10.47", $pluginPath);
        ...
        $environment->configure([
            ...
            "shortcode_name" => defined("SUPSYSTIC_TABLES_SHORTCODE_NAME")
                ? SUPSYSTIC_TABLES_SHORTCODE_NAME
                : $menuSlug,
            "shortcode_part_name" => defined(
                "SUPSYSTIC_TABLES_PART_SHORTCODE_NAME"
            )
                ? SUPSYSTIC_TABLES_PART_SHORTCODE_NAME
                : $menuSlug . "-part",
            "shortcode_cell_name" => defined(
                "SUPSYSTIC_TABLES_CELL_SHORTCODE_NAME"
            )
                ? SUPSYSTIC_TABLES_CELL_SHORTCODE_NAME
                : $menuSlug . "-cell-full",
            "shortcode_value_name" => defined(
                "SUPSYSTIC_TABLES_VALUE_SHORTCODE_NAME"
            )
                ? SUPSYSTIC_TABLES_VALUE_SHORTCODE_NAME
                : $menuSlug . "-cell",
            ...
            "revision" => 61,
            "welcome_page_was_showed" => get_option(
    ...
    public function db_table_exist($table)
    {
        ...
        switch ($table) {
            case "supsystic_tbl_tables":
                $res = $wpdb->get_var(
                    "SHOW TABLES LIKE '{$wpdb->prefix}supsystic_tbl_tables'"
                );
                break;
            case "supsystic_tbl_columns":
                $res = $wpdb->get_var(
                    "SHOW TABLES LIKE '{$wpdb->prefix}supsystic_tbl_columns'"
                );
                break;
            case "supsystic_tbl_conditions":
                $res = $wpdb->get_var(
                    "SHOW TABLES LIKE '{$wpdb->prefix}supsystic_tbl_conditions'"
                );
                break;
            case "supsystic_tbl_rows":
                $res = $wpdb->get_var(
                    "SHOW TABLES LIKE '{$wpdb->prefix}supsystic_tbl_rows'"
                );
                break;
            case "supsystic_tbl_diagrams":
                $res = $wpdb->get_var(
                    "SHOW TABLES LIKE '{$wpdb->prefix}supsystic_tbl_diagrams'"
                );
                break;
            case "supsystic_rows_history":
                $res = $wpdb->get_var(
                    "SHOW TABLES LIKE '{$wpdb->prefix}supsystic_rows_history'"
                );
                break;
        }
        return !empty($res);
    ...
    public function addShortcodeButton()
    {
        ...
        if (is_admin()) {
            wp_enqueue_script(
                "stb-bpopup-js",
                $this->environment->getConfig()->get("plugin_url") .
                    "/app/assets/js/plugins/jquery.bpopup.min.js",
                ["jquery"],
                false,
                true
            );
            wp_enqueue_style(
                "stb-bpopup",
                $this->environment->getConfig()->get("plugin_url") .
                    "/app/assets/css/editor-dialog.css"
            );
        }
    }
    ...
    {
        $plugin_array["addShortcodeDataTable"] =
            $this->environment->getConfig()->get("plugin_url") .
            "/app/assets/js/buttons.js";
